...
def integer(golden_lists,golden_lists_attr):
    ...
    for idx in range(0, sent_num):
        golden_list = golden_lists[idx]  # 一句话的标签

        golden_list_attr = golden_lists_attr[idx]  # 一句话的标签
...
def get_ner_fmeasure(golden_lists, predict_lists, label_type="BMES"):
    ...
    for idx in range(0, sent_num):
        ...
        # else:
        gold_matrix = get_ner_BIO(golden_list)  #不是序号了，是真实地标签
        pred_matrix = get_ner_BIO(predict_list)
        # print("gold", gold_matrix)
        # print("pred", pred_matrix)

        right_ner = list(set(gold_matrix).intersection(set(pred_matrix)))  #返回两个集合重合的元素
