...
class Data: 
    def __init__(self):
        ...
        self.attr_alphabet = Alphabet('attr', padflag=False,unkflag=False)
    ...
    def show_data_summary(self):
        ...
        print("     attr alphabet size: %s" % self.attr_alphabet.size())
        for attr, _ in self.attr_alphabet.iteritems():
            print(attr)
    ...
    def build_gaz_file(self, gaz_file, skip_first_row=False, separator=" "):
        ...
        # sgns.merge.word: 300-dimension vector corresponding to the words
        # print(gaz_file)
        if gaz_file:
            ...
            # obtain different words, then save them in self.gaz
        else:
    ...
    def generate_instance_new(self, input_char_file, input_word_file, input_label_file, name, random_split=False):

        # texts, ids = read_instance_new(input_char_file, input_word_file, input_label_file, input_attr_file,self.char_alphabet, self.label_alphabet,self.attr_alphabet, self.gaz_alphabet, self.number_normalized, self.max_sentence_length)
        texts, ids = read_instance_new(input_char_file, input_word_file, input_label_file, self.char_alphabet, self.label_alphabet,self.gaz_alphabet,self.number_normalized, self.max_sentence_length)
        if name == "train":
            # if random_split:
            #     random.seed(1)
